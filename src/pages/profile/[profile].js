...
export default function Post(props) {
    ...
    return (
        <div className="dark:bg-black">
            <Head title={  data.username + "'s posts on LinkHub" } desc={ "View " + data.username + "'s profile on LinkHub" } image="https://linkhub.arjunsivaraman.co/assets/seo/person.png"/>
                        ...
                        <div className="flex flex-row flex-nowrap justify-between items-center w-full px-4 gap-4">
                            ...
                            <div className="flex flex-col justify-start items-center gap-1 w-full">
                                <div className="flex flex-row gap-4 justify-start items-start w-full">
                                    ...
                                    { data.roles.includes("moderator") && <p className="self-center px-1.5 py-0.5 text-xs text-red-500 dark:text-red-400 sm:text-sm rounded-full border border-red-500 dark:border-red-400 font-semibold">Moderator</p> }
                                </div>
                                <p className="w-full text-left text-base md:text-lg xl:text-xl text-gray-500 dark:text-gray-300">{ "@" + data.username }</p>
                            </div>
                        </div>
                        <div className={(data.tags.length > 0 ? "flex " : "hidden ") + "flex-row flex-wrap gap-2 justify-start items-start w-full px-3 mt-4"}>
