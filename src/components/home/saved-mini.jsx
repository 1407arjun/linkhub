...
export default function SavedMini(props) {
    ...
    function updateSaved(postId) {
        setSaved((prev) => { return prev.filter(post => { return post._id.toString() !== postId })})
    }

    return (
        <div className={(saved.length > 0 ? "flex " : "hidden ") + "flex-col place-content-start gap-3 bg-gray-100 dark:bg-gray-900 rounded-3xl px-2 xl:px-4 py-4"}>
            ...
            <div className="flex flex-col place-content-start"> {/* Max 3 Strip before entering*/}
                { saved.map((data, index) => { return (
                    <Save key={ index } title={ data.title } author={ data.author.name } id={ data._id.toString() } update={ updateSaved }/>
                ) }) }
            </div>
