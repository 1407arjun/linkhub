...
export default function Dropdown(props) {
    ...
    async function updateSaved(newSaved) {
        setSaved(!newSaved)
        try {
            const r = await axios.post("/api/saved/update", {
                postId: props.id,
                remove: newSaved
            })
        ...
        <div className="flex flex-row justify-end items-center">
            ...
            {status === "authenticated" && (
                <button
                    ...
                    {saved && (
                        <img
                            src="/assets/posts/bookmark-done.svg"
                            className="w-4 xl:w-6"
                            alt="Saved"
                        />
                    )}
                    {!saved && (
                        <img
                            src="/assets/posts/bookmark.svg"
                            className="w-4 xl:w-6 dark:invert"
                            alt="Save"
                        />
                    )}
                </button>
